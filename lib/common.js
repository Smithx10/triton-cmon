...
function authorizationHandler(req, res, next) {
    ...
    if (req.host_prefix === DEFAULT_HOST_PREFIX) {
        ...
        if (accountVms.has(req.host_prefix)) {
            ...
            /* authenticated operator user is asking for instance metrics for
             * a differnet account
             * */
            next();
        } else {
...
function mapVm(vm, source, cb) {
    ...
    } else if (!(typeof (vm.billing_id) === 'string')) {
        err = new Error('vm.billing_id must be a string');
    } else if (!(typeof (vm.state) === 'string')) {
        err = new Error('vm.state must be a string');
    } else if (!(typeof (vm.last_modified) === 'string')) {
        err = new Error('vm.last_modified must be a string');
    } else if (!(typeof (vm.create_timestamp) === 'string')) {
        err = new Error('vm.create_timestamp must be a string');
    } else if (!(typeof (vm.autoboot) === 'boolean')) {
        err = new Error('vm.autoboot must be a boolean');
    } else if (!(typeof (vm.tags) === 'object')) {
        err = new Error('vm.tags must be a object');
    } else if (!(typeof (vm.customer_metadata) === 'object')) {
        err = new Error('vm.customer_metadata must be a object');
    } else if (!(typeof (vm.platform_buildstamp) === 'string')) {
        err = new Error('vm.platform_buildstamp must be a string');
    } else if (!(typeof (vm.owner_login) === 'string')) {
        err = new Error('vm.owner_login must be a string');
    } else if (!(typeof (vm.billing_name) === 'string')) {
        err = new Error('vm.billing_name must be a string');
    } else if (!(typeof (vm.firewall_enabled) === 'boolean')) {
        err = new Error('vm.firewall_enabled must be a boolean');
      /* 
       * This is not for Labels but for adding Metrics to the scrape.
       * We want to know this info even if the machine is state="stopped"
       * */
    } else if (!(typeof (vm.cpu_cap) === 'number')) {
        err = new Error('vm.cpu_cap must be a number');
    } else if (!(typeof (vm.cpu_shares) === 'number')) {
        err = new Error('vm.cpu_shares must be a number');
    } else if (!(typeof (vm.max_physical_memory) === 'number')) {
        err = new Error('vm.max_physical_memory must be a number');
    } else if (!(typeof (vm.max_swap) === 'number')) {
        err = new Error('vm.max_swap must be a number');
    } else if (!(typeof (vm.quota) === 'number')) {
        err = new Error('vm.quota must be a number');
    } else if (!(typeof (vm.max_lwps) === 'number')) {
        err = new Error('vm.max_lwps must be a number');
    } else if (!(typeof (vm.zfs_io_priority) === 'number')) {
        err = new Error('vm.zfs_io_priority must be a number');
    } else {
        ...
        if (vm.tags['smartdc_role'] || vm.tags['manta_role']) {
            has_core_tag = true;
            /*
             *  We don't scrape NAT zones
             */
            if (vm.tags['smartdc_role'] === "nat") {
                err = new Error('nat zones are skipped.');
            }
        }
        ...
        for (var metdata in vm.customer_metadata) {
            var stringMetadata = vm.customer_metadata[metdata].toString();
            vm.customer_metadata[metdata] = stringMetadata;
        }
        ...
        mappedVm =
        {
            ...
            vm_uuid: vm.uuid,
            vm_platform_buildstamp: vm.platform_buildstamp,
            vm_customer_metadata: vm.customer_metadata,
            vm_tags: vm.tags,
            vm_autoboot: vm.autoboot.toString(),
            vm_create_timestamp: vm.create_timestamp,
            vm_last_modified: vm.last_modified,
            vm_state: vm.state,
            vm_billing_id: vm.billing_id,
            vm_billing_name: vm.billing_name,
            vm_owner_login: vm.owner_login,
            vm_firewall_enabled: vm.firewall_enabled.toString(),
            max_lwps: vm.max_lwps,
            max_swap: vm.max_swap,
            max_physical_memory: vm.max_physical_memory,
            quota: vm.quota,
            zfs_io_priority: vm.zfs_io_priority,
