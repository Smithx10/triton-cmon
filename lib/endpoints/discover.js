...
function apiGetContainers(req, res, next) {
    ...
    if (account_is_cached && cache.owners.get(account_uuid).vms.size !== 0) {
        // Make sure if anyone requesting all (/discovery?all=true) is an operator.
        if (req.query && req.query['all'] === 'true' && req.account.isOperator) {
            var accountVms = cache.vms;
        } else {
            var accountVms = cache.owners.get(account_uuid).vms;
        }
        console.log("RAWR", cache.vms);
        var containerArray = [];
        mod_vasync.forEachPipeline({
