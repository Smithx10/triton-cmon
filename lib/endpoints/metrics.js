...
function fetchAndSendMetrics(req, res, req2, next, mstr) {
    req2.on('result', function _fetchMetrics(err2, res2) {
        ...
        res2.on('data', function _onData(chunk) {
            ...
            // Pop on the vmapi metrics
            var mStrBuf = Buffer.from(mstr, 'utf-8');
            data.push(mStrBuf);
...
function apiGetMetrics(req, res, next) {
    ...
    if (cache.admin_ips.has(target_uuid)) {
      ...
      /*
       * Create a string with infomation from the cache.
       */

      var fwEnabled
      if (vmCacheItem.vm_firewall_enabled === "true") {
          fwEnabled = 1;
      } else {
          fwEnabled = 0;
      }
        var mStr = `# HELP vmapi_max_physical_memory VMAPI max memory set
# TYPE vmapi_max_physical_memory gauage
vmapi_max_physical_memory ${vmCacheItem.max_physical_memory}
# HELP vmapi_max_swap VMAPI max swap set
# TYPE vmapi_max_swap gauage
vmapi_max_swap ${vmCacheItem.max_swap}
# HELP vmapi_zfs_quota VMAPI max zfs quota set
# TYPE vmapi_zfs_quota gauage
vmapi_zfs_quota ${vmCacheItem.quota}
# HELP vmapi_zfs_io_priority VMAPI zfs io priorty set
# TYPE vmapi_zfs_io_priority gauage
vmapi_zfs_io_priority ${vmCacheItem.zfs_io_priority}
# HELP vmapi_max_lwps VMAPI max light weight processes set
# TYPE vmapi_max_lwps gauage
vmapi_max_lwps ${vmCacheItem.max_lwps}
# HELP vmapi_cpu_cap VMAPI cpu_cap set
# TYPE vmapi_cpu_cap gauage
vmapi_cpu_cap ${vmCacheItem.cpu_cap}
# HELP vmapi_cpu_shares VMAPI cpu_shares set
# TYPE vmapi_cpu_shares gauage
vmapi_cpu_shares ${vmCacheItem.cpu_shares}
# HELP vmapi_firewall_eanbled VMAPI firewall set
# TYPE vmapi_firewall_eanbled gauage
vmapi_firewall_enabled ${fwEnabled}
`
      ...
      if (vmCacheItem.vm_state !== "running" && vmCacheItem.vm_state !== "destroyed") {
            //fetchAndSendMetrics(req, res, req2, next);
            var mStrBuf = Buffer.from(mStr, 'utf-8');
            res.end(mStrBuf);
            next();
        ...
        client.get(cmon_agent_path, function _onAgentConnect(err, req2) {
            ...
            fetchAndSendMetrics(req, res, req2, next, mStr);
        });
    } else {
